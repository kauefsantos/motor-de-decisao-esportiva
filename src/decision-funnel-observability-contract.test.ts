...
describe("decision funnel observability", () => {
  it("keeps observability read-only and derives the strict probability gate from the canonical helper", () => {
  ...
  it("does not use a pre-rule run as proof that the strict gate works in production", () => {
    const fn = source("./lib/decision-observability.functions.ts");
    expect(fn).toContain("STRICT_GATE_EFFECTIVE_AT");
    expect(fn).toContain("strictRuleHasRealRun");
    expect(fn).toContain("Ainda não existe uma análise iniciada depois da ativação da regra estrita >70%");
  });
  ...
  it("exposes the mobile-first diagnostic screen with the whole decision funnel", () => {
    ...
    expect(route).toContain("Passaram de 70%");
    expect(route).toContain("Bloqueadas em 70% ou menos");
    expect(route).toContain("EV automático ≥ 2%");
    expect(route).toContain("Cotação indevida em ≤70%");
