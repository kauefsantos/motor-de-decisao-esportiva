...
import { markAnalysisRunError } from "@/lib/repositories/analysis-worker.repository.server";
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        try {
          const { data: claimed, error: claimError } = await callAdminRuntimeRpc<ClaimedAnalysisJob[]>(
            "claim_analysis_job",
            { p_run_id: runId, p_dispatch_token: dispatchToken },
          );
          ...
          if (!nextStep) {
            const { data: finalRows } = await callAdminRuntimeRpc<AcceptedRow[]>(
              "complete_analysis_job_step_atomic",
              { p_run_id: runId, p_lease_token: leaseToken, p_step: null, p_finished: true },
            );
            const accepted = Boolean(finalRows?.[0]?.accepted);
            return backendJson({ status: accepted ? ("DONE" as const) : ("LEASE_LOST" as const), runId }, accepted ? undefined : { status: 202 }, requestId);
          }

          const { data: startedRows } = await callAdminRuntimeRpc<AcceptedRow[]>(
            "start_analysis_job_step_atomic",
            { p_run_id: runId, p_lease_token: leaseToken, p_step: nextStep.key },
          );
          if (!startedRows?.[0]?.accepted) {
          ...
          const heartbeat = async () => {
            ...
            heartbeatBusy = true;
            try {
              const { data, error } = await callAdminRuntimeRpc<boolean>("heartbeat_analysis_job", {
                p_run_id: runId,
          ...
          try {
            ...
            const accepted = Boolean(failedRows?.[0]?.accepted);
            if (accepted) await markAnalysisRunError(runId);
