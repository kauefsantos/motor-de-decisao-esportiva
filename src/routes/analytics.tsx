        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">
              Aqui você acompanha somente resultados já registrados, evolução da banca e qualidade das estimativas.
            </p>
          </div>
        ...
        {data && (
          <>
            ...
            <div className="mt-4 rounded-xl bg-secondary/25 px-4 py-3 text-sm leading-relaxed text-muted-foreground ring-1 ring-border/45">
              Para encerrar uma aposta, use <Link to="/open-bets" className="font-medium text-accent underline underline-offset-4">Em andamento</Link>. O histórico abaixo é somente leitura para evitar alterações duplicadas.
            </div>
                    ...
                    {data.byFamily.map((family) => (
                      <div key={family.family} className="border-b border-border/60 py-3 last:border-b-0">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <p className="num text-lg font-semibold text-primary">ROI {pct(family.roi)}</p>
                        </div>
            ...
            <CollapsiblePanel className="mt-4" title="Chances calculadas x resultados" description="Compare a estimativa com o que aconteceu de verdade" meta={data.calibration.length}>
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                {data.calibration.map((bucket) => (
                  <div key={bucket.bucket} className="border-b border-border/60 pb-3 last:border-b-0 sm:border-b-0 sm:pb-0"><p className="font-medium">{bucket.bucket}</p><p className="mt-2 text-xs text-muted-foreground">{bucket.count} caso(s)</p><div className="mt-2 flex justify-between gap-3 text-sm"><span>Calculado <span className="num">{pct(bucket.predicted)}</span></span><span>Real <span className="num">{pct(bucket.observed)}</span></span></div></div>
                ))}
              </div>
            </CollapsiblePanel>
                    ...
                    {recent.map((row) => (
                      <article key={row.id} className="border-b border-border/60 py-3 first:pt-0 last:border-b-0 last:pb-0">
                        ...
                        <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                          <div><dt className="text-muted-foreground">Odd final</dt><dd className="num mt-1 text-sm font-medium">{odd(row.closing_odd)}</dd></div>
                          <div><dt className="text-muted-foreground">Valor</dt><dd className="num mt-1 text-sm font-medium">{row.stake_brl === null ? "—" : money(Number(row.stake_brl))}</dd></div>
                          <div><dt className="text-muted-foreground">Resultado</dt><dd className="mt-1 text-sm font-medium">{friendlyResult(row.result)}</dd></div>
                          <div><dt className="text-muted-foreground">Financeiro</dt><dd className="num mt-1 text-sm font-medium">{row.profit_brl === null ? "—" : money(Number(row.profit_brl))}</dd></div>
                        </dl>
                      </article>
                    ))}
                  </div>
...
function MetricCard({ label, value, hint, highlight = false, help }: { label: string; value: string; hint: string; highlight?: boolean; help?: "ROI" | "CLV" }) {
  return (
    <div className={`panel p-4 ${highlight ? "border-primary/25 bg-primary/[0.055]" : ""}`}>
      <p className="flex items-center text-xs text-muted-foreground">{label}{help && <MetricHelp term={help} />}</p>
...
function MetricValue({ label, value, help }: { label: string; value: string; help?: "ROI" | "CLV" }) {
  return <div><p className="flex items-center text-xs text-muted-foreground">{label}{help && <MetricHelp term={help} />}</p><p className="num mt-1 text-base font-medium">{value}</p></div>;
}
  ...
  return (
    <div className="mt-4">
      <div className="grid grid-cols-2 gap-x-4 gap-y-3 sm:grid-cols-4">
        <MetricValue label="Início" value={money(start)} />
        <MetricValue label="Atual" value={money(current)} />
        <MetricValue label="Menor saldo" value={money(min)} />
        <MetricValue label="Maior saldo" value={money(max)} />
      </div>
