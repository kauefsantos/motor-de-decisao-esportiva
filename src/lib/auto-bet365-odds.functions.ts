  ...
  .handler(async ({ data, context }) => {
    ...
    for (const fetched of day.fetches) {
      await supabase.from("source_fetches").insert({
        run_id: data.runId, match_id: null, source: "five_dollar_bet365_day_odds",
    ...
    for (const matchId of matchIds) {
      ...
      if (!fixtureId || !Number.isFinite(fixtureId)) {
        for (const row of rows) {
          quotes.push({ predictionId: row.prediction_id, matchId, matchLabel, status: "SOURCE_UNAVAILABLE", odd: null, offeredLine: null, stage: null, apiMarket: null, reason: "Fixture ID da 5Dollar indisponível para esta partida." });
        }
        continue;
      }
      ...
      if (needsFull.size > 0) {
        ...
        await supabase.from("source_fetches").insert({
          run_id: data.runId, match_id: matchId, source: "five_dollar_bet365_odds",
      ...
      for (const row of rows) {
        ...
        if (!parsed && needsFull.has(row.prediction_id)) {
          ...
          parsed = fullFetch?.status === "OK" && fullFetch.payload !== null
            ...
            : { predictionId: row.prediction_id, status: "SOURCE_UNAVAILABLE" as const, odd: null, offeredLine: null, stage: null, apiMarket: null, reason: fullFetch?.errorMessage ?? `Falha ao consultar odds: ${fullFetch?.status ?? "sem resposta"}.` };
        }
        parsed ??= unsupported(row);
        quotes.push({ ...parsed, matchId, matchLabel });
        snapshotRows.push({
          run_id: data.runId, match_id: matchId, prediction_id: row.prediction_id, fixture_id: fixtureId,
    ...
    if (snapshotRows.length > 0) {
      for (let i = 0; i < snapshotRows.length; i += 200) {
        const { error } = await supabase.from("experimental_odds_snapshots").upsert(snapshotRows.slice(i, i + 200) as never[], { onConflict: "run_id,prediction_id" });
