...
export async function apiFootballGet<T = unknown>(path: string): Promise<ApiFootballFetch<T>> {
  ...
  if (!key) {
    return {
      ...
      payload: null,
      httpStatus: null,
      errorMessage: "API_FOOTBALL_KEY ausente no servidor.",
      fetchedAt: now(),
      fromCache: false,
    };
  }
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          ...
          errorMessage: `Credencial rejeitada pela API-Football (HTTP ${res.status}).`,
          fetchedAt: now(),
          fromCache: false,
        };
      }
      ...
      if (res.status === 429) {
        lastError = "Rate limit / cota da API-Football (HTTP 429).";
      } else if (!res.ok) {
        lastError = `HTTP ${res.status} ao consultar ${path}.`;
      } else {
        const payload = (await res.json()) as T;
        const errors = (payload as { errors?: unknown })?.errors;
        const errorList = errors && typeof errors === "object"
          ? Object.values(errors as object).filter(Boolean)
          : [];
        if (errorList.length > 0) {
          return {
            ...
            payload: null,
            httpStatus: res.status,
            errorMessage: `Erro reportado pela API-Football: ${errorList.join(" | ")}`,
            fetchedAt: now(),
            fromCache: false,
          };
        }
