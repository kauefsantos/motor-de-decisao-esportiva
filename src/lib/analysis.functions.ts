...
async function legacyIdempotencyKey(userId: string, data: z.infer<typeof createRunSchema>) {
  ...
  // absorbs double-click/network retries while still allowing an intentional
  // rerun of the same CSV shortly afterwards.
...
export const createRun = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: result, error } = await supabase.rpc("create_analysis_run_atomic", {
      p_owner_id: userId,
    ...
    return { runId: String(row.run_id), reused: Boolean(row.reused), steps: PIPELINE_STEPS.map((s) => ({ ...s })) };
  });
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: run, error: runError }, { data: candidates, error: candidateError }, { data: logs, error: logError }, { data: fetches, error: fetchError }] =
      await Promise.all([
        ...
        supabase.from("market_candidates")
        ...
        supabase.from("pipeline_logs").select("step, level, message, payload, created_at").eq("run_id", data.runId).order("created_at", { ascending: true }),
        supabase.from("source_fetches").select("source, status").eq("run_id", data.runId),
      ]);
    if (runError || candidateError || logError || fetchError) throw new BackendError("INTERNAL_ERROR", "Não foi possível carregar a análise completa.", 500);
...
export const analyzeOdds = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: candidates, error: candidateError }, { data: predictions, error: predictionError }] = await Promise.all([
      supabase.from("market_candidates")
        ...
        .eq("run_id", data.runId)
        .in("id", ids),
      supabase.from("model_predictions").select("prediction_id, outcome_distribution").eq("run_id", data.runId),
    ]);
    if (candidateError || predictionError) throw new BackendError("INTERNAL_ERROR", "Não foi possível preparar a comparação de odds.", 500);

    const distByPrediction = new Map(
      (predictions ?? []).map((prediction: any) => [prediction.prediction_id, (prediction.outcome_distribution ?? null) as AsianOutcomeProbabilities | null]),
    );
    ...
    const { error: persistError } = await supabase.rpc("replace_run_value_analysis_atomic", {
      p_run_id: data.runId,
      p_owner_id: userId,
      p_user_odds: oddsRows,
      p_evaluations: evaluationRows,
      p_selections: selectionRows,
    });
    if (persistError) throw new BackendError("CONFLICT", "As odds mudaram durante o processamento. Atualize e tente novamente.", 409);

    const rejected = evaluations
      .filter((item) => !selections.some((selection) => selection.candidateId === item.result.candidateId))
      .map((item) => ({ ...item.result, marketLabel: item.candidate.market_label, matchId: item.candidate.match_id }));
...
export const getResults = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: evaluations, error: evaluationError }, { data: selections, error: selectionError }] = await Promise.all([
      supabase.from("value_evaluations").select("*").eq("run_id", data.runId),
      supabase.from("final_selections").select("*").eq("run_id", data.runId).order("rank", { ascending: true }),
    ]);
    if (evaluationError || selectionError) throw new BackendError("INTERNAL_ERROR", "Não foi possível carregar os resultados.", 500);
    const ids = (evaluations ?? []).map((evaluation: any) => evaluation.candidate_id);
    const { data: candidates } = ids.length
      ? await supabase.from("market_candidates")
          .select("id, prediction_id, market_label, match_id, confidence_score, data_quality_score, settlement_definition, reason_short")
          .in("id", ids)
      : { data: [] };
    const { data: matches } = await supabase.from("matches").select("id, raw_partida, home_team, away_team, competition, kickoff_local").eq("run_id", data.runId);
...
export const getAudit = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [
      ...
      supabase.from("source_fetches").select("source, status, http_status, error_message, fetched_at, match_id").eq("run_id", data.runId),
      supabase.from("matches").select("id, raw_partida, home_team, away_team, competition, kickoff_local, resolution_status, resolution_reason, resolver_confidence").eq("run_id", data.runId),
      supabase.from("match_external_ids").select("match_id, source, external_id, confidence"),
      supabase.from("normalized_match_stats").select("match_id, scope, metric, normalized_value, sample_size, source, definition_version, lineage").eq("run_id", data.runId),
      supabase.from("raw_observations").select("match_id, source, metric, raw_value, definition_version").eq("run_id", data.runId),
      supabase.from("source_definitions").select("source, definition_version, notes, metric_definitions"),
    ]);

    const matchIds = new Set((matches ?? []).map((match: any) => match.id));
    const sources = new Map<string, { source: string; ok: number; unavailable: number; notConfigured: number; lastFetchedAt: string | null; lastError: string | null }>();
    for (const fetch of fetches ?? []) {
      const source = sources.get(fetch.source) ?? { source: fetch.source, ok: 0, unavailable: 0, notConfigured: 0, lastFetchedAt: null, lastError: null };
      if (fetch.status === "OK") source.ok += 1;
      else if (fetch.status === "NOT_CONFIGURED") source.notConfigured += 1;
      else source.unavailable += 1;
      if (!source.lastFetchedAt || (fetch.fetched_at && fetch.fetched_at > source.lastFetchedAt)) source.lastFetchedAt = fetch.fetched_at;
    ...
    const researchRaws = (raws ?? []).filter((raw: any) => raw.source === "research_adapter");
    const researchMode = (definitions ?? []).find((definition: any) => definition.source === "research_adapter")?.notes ?? "Desk Research / Dados Públicos";

    const perMatch = (matches ?? []).map((match: any) => {
      const mine = researchRaws.filter((raw: any) => raw.match_id === match.id);
      ...
      for (const raw of mine) {
        const value = (raw.raw_value ?? {}) as ResearchRaw;
        ...
        if (value.externalMatchId) fixtures.set(value.externalMatchId, { date: value.fixtureDate ?? "—", team: value.team ?? "—", opponent: value.opponent ?? "—" });
        if (value.contractCompatible === true) accepted.set(canonical, (accepted.get(canonical) ?? 0) + 1);
        else {
          const previous = rejected.get(canonical);
          rejected.set(canonical, { count: (previous?.count ?? 0) + 1, note: value.note ?? previous?.note ?? null });
        }
      }
      return {
        ...match,
        externalIds: (externalIds ?? []).filter((external: any) => external.match_id === match.id),
        normalized: (normalized ?? []).filter((row: any) => row.match_id === match.id),
    ...
    const researchActive = researchRaws.length > 0 || (fetches ?? []).some((fetch: any) => fetch.source === "research_adapter");
    return {
      mode: researchActive ? "Desk Research / Dados Públicos" : "APIs configuradas",
      sources: [...sources.values()].map((source) => ({
        ...source,
        status: source.ok > 0 ? (source.unavailable > 0 ? "PARTIAL" : "OK") : source.notConfigured > 0 && source.unavailable === 0 ? "NOT_CONFIGURED" : "UNAVAILABLE",
      })),
      resolvedEvents: (externalIds ?? []).filter((external: any) => external.source === "api_football_fixture" && matchIds.has(external.match_id)).length,
      researchResolved: new Set((externalIds ?? []).filter((external: any) => external.source === "research_team_home" && matchIds.has(external.match_id)).map((external: any) => external.match_id)).size,
      normalizedObservations: (normalized ?? []).length,
      rawObservations: (raws ?? []).length,
      crossChecked: (normalized ?? []).filter((row: any) => ((row.lineage as { crossCheck?: { status?: string } } | null)?.crossCheck?.status ?? "") === "CROSS_SOURCE_CONFIRMED").length,
      sourceConflicts: (normalized ?? []).filter((row: any) => ((row.lineage as { crossCheck?: { status?: string } } | null)?.crossCheck?.status ?? "") === "SOURCE_CONFLICT").length,
      matches: perMatch,
    };
  });
