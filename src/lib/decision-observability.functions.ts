...
// Nova régua do funil: probabilidade >=70%, odd >=1,70, EV >=8%, edge >=5 p.p.
// Runs anteriores ficam como evidência histórica e não validam a regra nova.
export const MARKET_FUNNEL_EFFECTIVE_AT = "2026-09-12T19:00:00.000Z";
export const STRICT_GATE_EFFECTIVE_AT = MARKET_FUNNEL_EFFECTIVE_AT;
...
async function buildFunnel(rawDb: { from: (table: string) => any }, run: RunRow) {
  const [{ data: predictionData }, { data: snapshotData }, { data: trackingData }] = await Promise.all([
    ...
    rawDb
      ...
      .select("prediction_id,model_probability,entry_odd,edge,expected_value,bet_status,result")
      .eq("run_id", run.id),
  ]);
  ...
  const rejectedBelow70 = probabilities.length - above70;

  const matchedSnapshots = snapshots.filter(
    (row) => row.status === "MATCHED" && (numberOrNull(row.odd) ?? 0) > 1,
  );
  const automaticOddFloorPass = matchedSnapshots.filter((row) => passesMinimumOddGate(numberOrNull(row.odd))).length;
  const pricedBelow70 = matchedSnapshots.filter((row) => {
  ...
  const automaticEvPass = matchedSnapshots.filter((row) => {
    ...
    if (!prediction || probability === null || odd === null) return false;
    ...
    return evaluated.executionStatus === "EXECUTAVEL";
  }).length;

  const selectedOutsideRule = tracking.filter((row) => {
    const p = numberOrNull(row.model_probability);
    const odd = numberOrNull(row.entry_odd);
    const edge = numberOrNull(row.edge);
    const ev = numberOrNull(row.expected_value);
    return !passesExperimentalModelGate(p) || !passesMinimumOddGate(odd) || (edge ?? -Infinity) < 0.05 || (ev ?? -Infinity) < 0.08;
  }).length;

  return {
    anchorPredictions: anchors.length,
    above70,
    // Nome mantido por compatibilidade; semanticamente agora representa somente <70%.
    rejectedAtOrBelow70: rejectedBelow70,
    ...
    pricedAtOrBelow70: pricedBelow70,
    automaticEvPass,
...
export const getDecisionObservability = createServerFn({ method: "GET" }).handler(async ({ context }) => {
  ...
  const strictRun = runs.find((run) => run.created_at >= MARKET_FUNNEL_EFFECTIVE_AT) ?? null;
  ...
  const health = {
    ...
    noSelectionOutsideRule: isStrictValidation ? funnel.selectedOutsideRule === 0 : null,
    readyForValueValidation: funnel.automaticPrices > 0 || funnel.selected > 0,
  };

  return {
    effectiveAt: MARKET_FUNNEL_EFFECTIVE_AT,
    inspectedRun,
    isStrictValidation,
    selectionLimit,
    funnel,
    ...
    recentRuns: runs.map((run) => ({
      ...
      afterStrictGate: run.created_at >= MARKET_FUNNEL_EFFECTIVE_AT,
    })),
    note: isStrictValidation
      ? "Esta análise foi iniciada depois da ativação da régua 70% + odd 1,70 + value."
      : "Ainda não existe uma análise iniciada depois da ativação da nova régua; os números exibidos são apenas referência histórica.",
  };
});
