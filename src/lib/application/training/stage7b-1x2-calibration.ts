...
export const STAGE7B_1X2_CALIBRATION_PROTOCOL = "stage7b-1x2-robust-calibration-v1";
export const STAGE7B_1X2_HOLDOUT_PROTOCOL = "stage7b-1x2-prospective-holdout-v1";
export const STAGE7B_1X2_TARGET_MODEL = STAGE6_GOALS_ELO_ARTIFACT;
export const STAGE7B_1X2_CALIBRATION_VERSION = "isotonic-classwise-v1-fit-through-2026-05-31";
export const STAGE7B_INTERNAL_SELECTION_START = "2026-04-01";
export const STAGE7B_CALIBRATION_END_EXCLUSIVE = "2026-06-01";
export const STAGE7B_RETROSPECTIVE_END_EXCLUSIVE = "2026-09-14";
export const STAGE7B_PROSPECTIVE_HOLDOUT_START = "2026-09-14";
export const STAGE7B_MIN_FIT_SAMPLE = 1000;
export const STAGE7B_MIN_SELECTION_SAMPLE = 300;
export const STAGE7B_MIN_RETROSPECTIVE_SAMPLE = 300;
export const STAGE7B_MIN_PROSPECTIVE_HOLDOUT_SAMPLE = 200;

const BIN_COUNTS = [15, 25, 40, 60] as const;
  ...
  return rows.map((row) => {
    ...
    const probabilities = kind === "raw"
      ...
      : kind === "baseline"
        ...
        : parameters
          ? applyOneXTwoClasswiseIsotonic(raw, parameters)
          : raw;
...
function selectHyperparameters(
  ...
  for (const binCount of BIN_COUNTS) {
    const base = fitOneXTwoClasswiseIsotonic(fitInputs, binCount);
    for (const blend of BLENDS) {
      const parameters: OneXTwoIsotonicParameters = { ...base, blend };
  ...
  if (!selected) throw new Error("Stage 7B could not select an isotonic calibration candidate.");
...
export function runStage7BOneXTwoCalibration(sourceRows: readonly Stage6GoalRow[]) {
  const predictions = buildStage6GoalsPredictions(sourceRows, STAGE7B_1X2_TARGET_MODEL);
  const fitRows = predictions.filter((row) => row.date < STAGE7B_INTERNAL_SELECTION_START);
  const selectionRows = predictions.filter(
    (row) => row.date >= STAGE7B_INTERNAL_SELECTION_START && row.date < STAGE7B_CALIBRATION_END_EXCLUSIVE,
  );
  const fullCalibrationRows = predictions.filter((row) => row.date < STAGE7B_CALIBRATION_END_EXCLUSIVE);
  const retrospectiveRows = predictions.filter(
    (row) => row.date >= STAGE7B_CALIBRATION_END_EXCLUSIVE && row.date < STAGE7B_RETROSPECTIVE_END_EXCLUSIVE,
  );

  if (fitRows.length === 0 || selectionRows.length === 0 || fullCalibrationRows.length === 0) {
    return {
      protocolVersion: STAGE7B_1X2_CALIBRATION_PROTOCOL,
      marketFamily: "1X2" as const,
      targetArtifact: STAGE7B_1X2_TARGET_MODEL,
      calibrationVersion: STAGE7B_1X2_CALIBRATION_VERSION,
      readinessStatus: "INSUFFICIENT_DATA" as Stage7Readiness,
      calibrationFit: null,
      retrospective: null,
      prospectiveHoldout: { startsAt: STAGE7B_PROSPECTIVE_HOLDOUT_START, requiredFixtures: STAGE7B_MIN_PROSPECTIVE_HOLDOUT_SAMPLE, status: "NOT_STARTED" as const },
      promotion: { productionValidated: false, reason: "Stage 7B fitting or internal selection window is empty." },
    };
  }

  const selection = selectHyperparameters(fitRows, selectionRows);
  const refitBase = fitOneXTwoClasswiseIsotonic(
    calibrationInputs(fullCalibrationRows),
    selection.selected.binCount,
  );
  const parameters: OneXTwoIsotonicParameters = {
    ...refitBase,
  ...
  const enoughFitData = fitRows.length >= STAGE7B_MIN_FIT_SAMPLE;
  const enoughSelectionData = selectionRows.length >= STAGE7B_MIN_SELECTION_SAMPLE;
  const enoughRetrospectiveData = retrospectiveRows.length >= STAGE7B_MIN_RETROSPECTIVE_SAMPLE;
  ...
  return {
    ...
    retrospective: {
      ...
      stabilityByPeriod,
      acceptance: {
        minimumFitSample: STAGE7B_MIN_FIT_SAMPLE,
        minimumSelectionSample: STAGE7B_MIN_SELECTION_SAMPLE,
        minimumRetrospectiveSample: STAGE7B_MIN_RETROSPECTIVE_SAMPLE,
    ...
    prospectiveHoldout: {
      startsAt: STAGE7B_PROSPECTIVE_HOLDOUT_START,
      requiredFixtures: STAGE7B_MIN_PROSPECTIVE_HOLDOUT_SAMPLE,
      status: readinessStatus === "SHADOW_READY" ? "PENDING" as const : "NOT_STARTED" as const,
    },
    promotion: {
      productionValidated: false,
      reason: readinessStatus === "SHADOW_READY"
        ? "Stage 7B retrospective gate passed. The frozen calibrator must pass the untouched prospective holdout before any governed production promotion."
        : "Stage 7B robust calibration did not pass every retrospective prerequisite; no production promotion is allowed.",
    },
  };
}
...
export function runStage7BProspectiveHoldout(rows: readonly Stage7HoldoutFixture[]) {
  const eligible = rows
    .filter((row) => row.fixtureDate >= STAGE7B_PROSPECTIVE_HOLDOUT_START)
  ...
  const enoughData = eligible.length >= STAGE7B_MIN_PROSPECTIVE_HOLDOUT_SAMPLE;
  ...
  const stabilityByLeague = [...byLeague.entries()]
    ...
    .map(([key, group]) => ({ key, predictions: group.length, metrics: multiclassMetrics(holdoutMetricInputs(group, "calibrated")) }))
    .sort((a, b) => a.key.localeCompare(b.key));
  ...
  const readinessStatus: Stage7Readiness = !enoughData
    ...
    : improvesOrPreservesBrier && improvesOrPreservesLogLoss && calibrationWithinTolerance && enoughStabilityCoverage
      ? "HOLDOUT_PASSED"
      : "HOLDOUT_FAILED";

  return {
    protocolVersion: STAGE7B_1X2_HOLDOUT_PROTOCOL,
    marketFamily: "1X2" as const,
    targetArtifact: STAGE7B_1X2_TARGET_MODEL,
    calibrationVersion: STAGE7B_1X2_CALIBRATION_VERSION,
    readinessStatus,
    ...
    promotion: {
      ...
      reason: readinessStatus === "HOLDOUT_PASSED"
        ? "Stage 7B untouched prospective holdout passed. Explicit governed promotion is still required."
        : readinessStatus === "PROSPECTIVE_HOLDOUT_PENDING"
          ? "Stage 7B untouched prospective holdout has not reached the minimum sample."
          : "Stage 7B untouched prospective holdout failed at least one production prerequisite.",
    },
  };
}
