...
-- Browser access is limited by RLS; server-only secrets/jobs stay service-role only.
...
revoke all on table public.app_server_secrets from public, anon, authenticated;
grant all on table public.app_server_secrets to service_role;
...
declare
  ...
  request_id bigint;
begin
  select value into worker_url from public.app_server_secrets where key = 'analysis_worker_url';
  select value into worker_token from public.app_server_secrets where key = 'analysis_worker_token';

  if worker_url is null or worker_token is null then
    return null;
  end if;
