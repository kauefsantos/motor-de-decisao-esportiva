-- Stage 1 runtime reconciliation.
-- Forward-only and idempotent: restores the performance/scalability objects that
-- are versioned in GitHub but were not fully present in Lovable Cloud.
-- Business/model/decision thresholds are intentionally unchanged.
...
begin
  ...
  where sf.run_id=r.id
    and r.created_at < pg_catalog.now() - interval '180 days'
    and (r.selection_finalized_at is not null or r.status='COMPLETED');
  ...
  delete from public.performance_vitals
...
begin
  ...
  if exists(select 1 from public.analysis_jobs where status in ('QUEUED','RUNNING')) then
    return pg_catalog.jsonb_build_object('status','SKIPPED','reason','analysis_busy');
  end if;
...
as $$
begin
  if not pg_catalog.pg_try_advisory_xact_lock(pg_catalog.hashtext('external_api_maintenance_when_idle')) then
    return pg_catalog.jsonb_build_object('status','SKIPPED','reason','already_running');
  end if;
  if exists(select 1 from public.analysis_jobs where status in ('QUEUED','RUNNING')) then
    return pg_catalog.jsonb_build_object('status','SKIPPED','reason','analysis_busy');
...
select cron.schedule(
  'elo-daily-incremental',
  '*/2 6-7 * * *',
  'select public.elo_sync_next_target_when_idle();'
);
select cron.schedule(
  'five-dollar-maintenance-daily',
  '10,25,40,55 6 * * *',
  'select public.kick_external_api_maintenance_when_idle();'
);
select cron.schedule(
  'performance-retention-daily',
  '15 4 * * *',
  'select public.run_performance_retention_cleanup();'
);
...
values
  (
    ...
    'Reconciled into Lovable Cloud by the Stage 1 forward-only runtime migration.'
  ),
  (
  ...
  (
    '20260913-performance-runtime-reconciliation',
    'reconcile_performance_runtime',
    'Forward-only reconciliation of missing performance indexes, owner-scoped helpers, Web Vitals storage, retention and idle-aware maintenance schedules.'
  )
