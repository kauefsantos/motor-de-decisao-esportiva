-- Final preflight repair for the scheduled D+2 dispatcher.
-- automation_runs.request_id is protected by a partial unique index
-- (WHERE request_id IS NOT NULL), so PostgreSQL cannot infer that index from
-- ON CONFLICT(request_id). Keep the ledger idempotent with bare ON CONFLICT,
-- matching the already-hardened analysis, maintenance and push dispatchers.
...
begin
  ...
  values(
    ...
    v_request,
    pg_catalog.jsonb_build_object('localZone','America/Sao_Paulo')
  )
...
values(
  '20260913-scheduled-d2-dispatch-conflict-fix',
  'scheduled_d2_dispatch_conflict_fix',
  'Preflight repair: scheduled D+2 dispatch now uses bare ON CONFLICT DO NOTHING, compatible with the partial unique request_id index. Scheduling, target date, fixture scope and business rules are unchanged.'
)
